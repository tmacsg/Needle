...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_strides = tuple([0 if self.shape[i] != new_shape[i] else self.strides[i] 
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = [idxs[-1].step * self.strides[-1]] * len(self.shape)
        for i in reversed(range(len(self.shape)-1)):
            new_strides[i] = idxs[i].step * self.strides[i]
        return NDArray.make(new_shape, strides=tuple(new_strides), device=self.device, handle=self._handle, offset=offset)
        ### END YOUR SOLUTION
    ...
    def sigmoid(self):
        out = NDArray.make(self.shape, device=self.device)
        self.device.ewise_sigmoid(self.compact()._handle, out._handle)
        return out
    ...
    def flip(self, axes):
        ...
        size = list(self._strides)
        # size[len(size)-1] = 1
        for i in range(len(size)):
            size[i] = size[i] * (self.shape[i]-1)
 
        for axis in axes:
            new_strides[axis] = -new_strides[axis] 
            offset += size[axis]
        array = NDArray.make(shape=self.shape, 
    ...
    def pad(self, axes):
        ...
        for i, axis in enumerate(axes):
            slices.append(slice(axis[0], axis[0]+new_shape[i]))
